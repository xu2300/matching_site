...
switch ($field) {
	...
	case "password":
		...
		break;
	case "custom_profile_fields_age":
	    $result["status"] = true;
	    $result["name"] = "custom_profile_fields_age";
		$bithdayDate = get_input("custom_profile_fields_age");
		$date = new DateTime($bithdayDate);
 		$now = new DateTime();
 		$interval = $now->diff($date);
 		if($interval->y < 13){
 			$result["status"] = false;
 			$result["text"] = elgg_echo("profile_manager:register_pre_check:PG13");
 		}
		break;
}
